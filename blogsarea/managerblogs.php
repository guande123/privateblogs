...
<link rel="stylesheet" type="text/css" href="../css/content.css" />	
     ...
     <header>
	   <ul>
		...
		if(isset($_SESSION['res'])){
			if(!$_SESSION['res']->isManager){//不是管理员就去用户界面
			 ...
			 <div class="nav_r">
			    <div class="nav_r_wrap">
					...
					<ul>
						...
						<li><a id="a_hide_user" href="javascript:void(0);" onclick="">用户管理</a></li>
					</ul>
				</div>
			</div>
	   </div>
  ...
  <script type="text/javascript">
	...
	var allUser =  document.getElementById("a_hide_user");
	var page_count = 0;
	function next(){
	...
	function xiajia(id){
	   var reason = prompt("下架理由.");
	   if(reason==null||reason==""){
		    return;
	   }
		url="xiaJiaServer.php?&id="+id;
		url = url+"&reason="+reason;	
		connectServer(url);
	}
	function getAllUser(){
		url="userServlet.php?count=0";
		connectServer(url);
	}
	function hideOrShow(num,id){
		url="userServlet.php?count=0&hide="+num+"&id="+id;
		connectServer(url);
	}
